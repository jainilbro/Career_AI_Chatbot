...
def initialize_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS chat_history (
            username TEXT PRIMARY KEY, history_data BLOB NOT NULL,
...
def transform_query_for_search(user_query: str) -> str:
    ...
    Remove any conversational fluff. Today's date is August 25, 2025.
...
# --------------------------------------------------
# --- REPLACE your old sidebar block with this new one ---
with st.sidebar:
    st.title("Welcome")
...
with col1:
    st.markdown(
        f"""
        <img src="data:image/png;base64,{logo_base64}" width="77" style="display:block; margin:auto;">
        """,
        unsafe_allow_html=True
    )
        ...
        if user:
            ...
            if check_password(password, hashed_password):
                ...
                if not st.session_state.history:
                    # If no history, create the very first "New Chat"
                    new_chat_id = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                    ...
                    if len(st.session_state.history[most_recent_chat_id]["messages"]) > 1:
                        ...
                        st.session_state.current_cat_id = most_recent_chat_id
...
if user_input := st.chat_input("Send a message..."):
    ...
    # First, determine the correct chat_id
    if st.session_state.current_chat_id is None:
        # If there's no active chat, create a new one
        chat_id = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        st.session_state.current_chat_id = chat_id
        st.session_state.history[chat_id] = {
            "title": user_input, # Use the full input as the title
            "messages": [SYSTEM_PROMPT]
        }
    else:
        # If there is an active chat, use its ID
        chat_id = st.session_state.current_chat_id
        # If it's an empty "New Chat", update its title with the first message
        if st.session_state.history[chat_id]["title"] == "New Chat":
            st.session_state.history[chat_id]["title"] = user_input

    # Now that we have a guaranteed chat_id, append the user message
