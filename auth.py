# auth.py (clean, Turso/libsql sync client)
...
def _to_wss(url: str) -> str:
    """
    Coerce common forms to a secure WebSocket URL for the libsql client.
    Accepts wss://, ws://, libsql://, https://, http://, or bare host.
    """
    u = str(url).strip()
    if u.startswith("wss://"):
        return u
    ...
    if u.startswith("libsql://"):
        return "wss://" + u[len("libsql://"):]
    if u.startswith("https://") or u.startswith("http://"):
        return "wss://" + u.split("://", 1)[1]
    return "wss://" + u

def get_db_connection():
    """
    Create a sync client for Turso/libsql using Streamlit secrets.
    Raises a clear error if secrets are missing/invalid.
    """
    try:
...
def ensure_db_ready():
    ...
    try:
        ...
        # ensure schema (matches your local create_db.py)
        conn.batch(
            [
